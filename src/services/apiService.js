...
async function handleNetworkCall(apiObject) {
  ...
  body = apiObject.body ? JSON.stringify(apiObject.body) : null;
  ...
  if (isAuthenticationRequired) {
    ...
    fetchObject.headers = { ...fetchObject.headers, 'Authorization': `Bearer ${accessToken}` };
  }
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (err.responseStatus === httpStatus.UNAUTHORIZED) {
        console.error('Login Token expired, please Login again.');
